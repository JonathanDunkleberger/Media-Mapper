...
export default async function MediaDetailPage(props: unknown) {
  ...
  const media: KnownMedia = await fetch(`http://localhost:3001/api/media/${category}/${slug}`).then(res => res.json());
  const similar: KnownMedia[] = await fetch(`http://localhost:3001/api/media/${category}/${slug}/similar`).then(res => res.json());

  return (
    <main className="bg-black min-h-screen text-white">
      <section className="flex flex-col md:flex-row gap-8 p-8">
        <img src={getImageUrl(media as KnownMedia) ?? ''} alt={getTitle(media as KnownMedia)} className="w-64 h-96 rounded-lg shadow-lg" />
        <div>
          <h1 className="text-4xl font-bold mb-2">{getTitle(media as KnownMedia)}</h1>
