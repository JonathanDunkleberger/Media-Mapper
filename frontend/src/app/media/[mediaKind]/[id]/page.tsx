...
interface MediaDetailPageProps { params?: Promise<{ mediaKind?: string; id?: string }>; }

interface TmdbMovieOrTvBase { id: number; title?: string; name?: string; poster_path?: string | null; overview?: string | null; }
...
export default async function MediaDetailPage({ params }: MediaDetailPageProps) {
  ...
  const type = resolved.mediaKind;
  const id = resolved.id;
  if (!type || !id) return <div className="p-8 text-sm text-red-400">Missing route params.</div>;
  const data = await fetchDetail(type, id);
  if (!data) return <div className="p-8 text-sm text-red-400">Not found.</div>;
